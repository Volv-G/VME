...
class ScoreboardOverlay:
    ...
    def apply(self, frame: np.ndarray, state: GameState) -> np.ndarray:
        h, w = frame.shape[:2]
        overlay = self.render_overlay(h, state).copy()
        alpha = overlay.split()[3].point(lambda v: int(v * OVERLAY_OPACITY))
        overlay.putalpha(alpha)

        video = Image.fromarray(frame).convert("RGBA")
        x = (w - overlay.width) // 2
        y = h - overlay.height - 30
        video.paste(overlay, (x, y), overlay)
